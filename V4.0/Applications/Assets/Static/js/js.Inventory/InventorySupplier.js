...
$(function (){
    ...
    $(body).on('activate', '#new-supplier, #modify-supplier', function (){
        let bool = true;
    ...
    $(body).on('change', 'select#civilities', function (){
        ...
        //
        $('#new-supplier input').removeClass('ts-disabled').prop('disabled', false);
        $('#new-supplier select:not(#civilities)').find('option:first-of-type').prop('selected', true);
        switch (txt){
            case 'non applicable':
    ...
    $(body).on('click', '#supplier-delete-list .bi-trash', function (){
        let url = '../../Inventory/Config/RemoveSupplier', id = $(this).parent().attr('data-id');
        ...
        $.ajax({
            ...
            success: function(response) {
                $(body).find('#supplier-delete-list [data-id="' + id + '"]').remove();
            },
    ...
    $('#main-content').on('click', '#supplier-list .ts-elt', function (){
        let id = $(this).attr('data-id'), form = $(this).parent().prev();
        $.loadResources('Inventory', 'Config', 'LoadSupplier', {_supplierId: id})
            .then(data => {
                console.log(data);
            })
            .catch(error => {
