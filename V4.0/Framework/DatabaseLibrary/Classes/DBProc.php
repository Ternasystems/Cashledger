...
class DBProc extends DBPDO
{
    ...
    public function QueryPDO(PDO $_pdo, ?array $_arg = null, int $_fetchMode = 0, ?array $_options = null) : array{
        $bool = (bool)preg_match('/^call/i', $this->GetStatement());
        if(!$bool){
            $messages = array(
                'en' => '<b>'.strtoupper('call').'</b> statement required.',
                'fr' => 'Requête <b>'.strtoupper('call').'</b> attendue.'
            );
