...
class CollectableException extends AbstractException
{
    /**
     * @throws Exception
     */
    public function SetMessages(array $_messages) : void{
        ...
        foreach($indexes as $index){
            if(!is_string($index) || strlen($index) != 2){
                $msg = 'A CollectableException method (SetMessages) received an array with non-string keys. Exception messages array must be a key/value array where keys are all of type string(2)';
    ...
    public function SetMessage(string $_lang, string $_message) : void{
        ...
        if(strlen($_lang) != 2){
            $msg = 'A CollectableException method (SetMessage) received a wrong argument (lang). Lang argument must be of type string(2).';
            $this->InternalException($msg);
        }
    ...
    public function GetMessageValue(string $_lang) : string{
        ...
        if(strlen($_lang) != 2 || !array_key_exists($_lang, $this->messages)){
            $msg = 'A CollectableException method (getMessageValue) received a wrong argument (lang). Lang argument must be of type string(2) and of one of the following value: '.join(',', array_keys($this->messages));
            $this->InternalException($msg);
        }

        // Return the Exception message
        return 'CollectableException ('.get_class($this->exception).'): '.$this->messages[$_lang];
    }
}