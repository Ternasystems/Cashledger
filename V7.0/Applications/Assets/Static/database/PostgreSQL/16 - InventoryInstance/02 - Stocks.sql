...
BEGIN
	-- Format sql
	_sql := FORMAT('INSERT INTO public.%I VALUES (%s, %L, %L, %L, %L, NOW(), %L, NOW(), %L, %L, NULL, %L) ON CONFLICT ("ProductID", "WarehouseID", "PackagingID", "BatchNumber") DO NOTHING;', _tablename, _id,
	_productid, _unitid, _warehouseid, _packagingid, _batchnumber, _quantity, _unitcost, _description);
	-- Execute sql
...
BEGIN
	-- Format sql
	_sql := FORMAT('INSERT INTO public.%I VALUES (%s, %L, %L, %L, NULL, %L) ON CONFLICT ("AttributeID", "StockID", "Value") DO NOTHING;', _tablename, _id, _attributeid, _stockid, _value, _description);
	-- Execute sql
