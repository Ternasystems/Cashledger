...
{
    protected ITaxService $service;

    public function __construct(ITaxService $service)
    {
    ...
    /**
     * Gets a paginated list of taxes.
     * Supports filtering via query string.
     * e.g., /?controller=Tax&action=index&filter[Label_like]=VAT%
     */
    public function index(Request $request): Response
    {
        try {
            ...
            $result = $this->service->getTaxes($filter, $page, $pageSize, $reload);
            return $this->json($result);

        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve taxes.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Handles requests for a single tax by its ID.
     * e.g., /?controller=Tax&action=show&id=tax-uuid-123
     */
    public function show(Request $request): Response
    {
        ...
        try {
            $filter = [['Id', '=', $id]];
            $result = $this->service->getTaxes($filter, 1, 1, ReloadMode::YES);
            return $this->json($result);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve tax.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Tax&action=store
     */
    public function store(Request $request): Response
    {
        ...
        try {
            $item = $this->service->SetTax($data);
            return $this->json($item, 201); // 201 Created
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to create tax.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Tax&action=update&id=...
     */
    public function update(Request $request): Response
    {
        ...
        try {
            $item = $this->service->PutTax($id, $data);
            if (!$item) {
                return $this->json(['error' => 'Tax not found.'], 404);
            }
            return $this->json($item);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to update tax.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Deletes (deactivates) a tax by its ID.
     * e.g., /?controller=Tax&action=destroy&id=...
     */
    public function destroy(Request $request): Response
    {
        ...
        try {
            ...
            return $this->json(['error' => 'Failed to delete tax.'], 500);
        } catch (Exception $e) {
