...
{
    protected IDiscountService $service;

    public function __construct(IDiscountService $service)
    {
    ...
    /**
     * Gets a paginated list of discounts.
     * Supports filtering via query string.
     * e.g., /?controller=Discount&action=index&filter[Label_like]=SUMMER%
     */
    public function index(Request $request): Response
    {
        try {
            ...
            $result = $this->service->getDiscounts($filter, $page, $pageSize, $reload);
            return $this->json($result);

        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve discounts.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Handles requests for a single discount by its ID.
     * e.g., /?controller=Discount&action=show&id=discount-uuid-123
     */
    public function show(Request $request): Response
    {
        ...
        try {
            $filter = [['Id', '=', $id]];
            $result = $this->service->getDiscounts($filter, 1, 1, ReloadMode::YES);
            return $this->json($result);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve discount.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Discount&action=store
     */
    public function store(Request $request): Response
    {
        ...
        try {
            $item = $this->service->setDiscount($data);
            return $this->json($item, 201); // 201 Created
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to create discount.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Discount&action=update&id=...
     */
    public function update(Request $request): Response
    {
        ...
        try {
            $item = $this->service->putDiscount($id, $data);
            if (!$item) {
                return $this->json(['error' => 'Discount not found.'], 404);
            }
            return $this->json($item);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to update discount.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Deletes (deactivates) a discount by its ID.
     * e.g., /?controller=Discount&action=destroy&id=...
     */
    public function destroy(Request $request): Response
    {
        ...
        try {
            ...
            return $this->json(['error' => 'Failed to delete discount.'], 500);
        } catch (Exception $e) {
