...
class CurrencyController extends BaseController
{
    ...
    public function __construct(ICurrencyService $service)
    {
    ...
    /**
     * Gets a paginated list of currencies.
     * Supports filtering via query string.
     * e.g., /?controller=Currency&action=index&filter[Label_like]=USD%
     */
    public function index(Request $request): Response
    {
        try {
            ...
            $result = $this->service->getCurrencies($filter, $page, $pageSize, $reload);
            return $this->json($result);

        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve currencies.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Handles requests for a single currency by its ID.
     * e.g., /?controller=Currency&action=show&id=currency-uuid-123
     */
    public function show(Request $request): Response
    {
        ...
        try {
            $filter = [['Id', '=', $id]];
            $result = $this->service->getCurrencies($filter, 1, 1, ReloadMode::YES);
            return $this->json($result);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve currency.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Currency&action=store
     */
    public function store(Request $request): Response
    {
        ...
        try {
            $item = $this->service->setCurrency($data);
            return $this->json($item, 201); // 201 Created
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to create currency.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Currency&action=update&id=...
     */
    public function update(Request $request): Response
    {
        ...
        try {
            $item = $this->service->putCurrency($id, $data);
            if (!$item) {
                return $this->json(['error' => 'Currency not found.'], 404);
            }
            return $this->json($item);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to update currency.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Deletes (deactivates) a currency by its ID.
     * e.g., /?controller=Currency&action=destroy&id=...
     */
    public function destroy(Request $request): Response
    {
        ...
        try {
            $success = $this->service->deleteCurrency($id);
            if ($success) {
                return new Response('', 204); // 204 No Content
            }
            return $this->json(['error' => 'Failed to delete currency.'], 500);
        } catch (Exception $e) {
