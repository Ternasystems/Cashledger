...
{
    protected IStockService $service;

    public function __construct(IStockService $service)
    {
    ...
    /**
     * Gets a paginated list of stock items.
     * Supports filtering via query string.
     * e.g., /?controller=Stock&action=index&filter[ProductId]=...
     */
    public function index(Request $request): Response
    {
        try {
            ...
            $result = $this->service->getStocks($filter, $page, $pageSize, $reload);
            return $this->json($result);

        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve stock items.', 'message' => $e->getMessage()], 500);
        }
    }

    /**
     * Handles requests for a single stock item by its ID.
     * e.g., /?controller=Stock&action=show&id=stock-uuid-123
     */
    public function show(Request $request): Response
    {
        ...
        try {
            $filter = [['Id', '=', $id]];
            $result = $this->service->getStocks($filter, 1, 1, ReloadMode::YES);
            return $this->json($result);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to retrieve stock item.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Stock&action=store
     */
    public function store(Request $request): Response
    {
        ...
        try {
            $item = $this->service->SetStock($data);
            return $this->json($item, 201); // 201 Created
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to create stock item.', 'message' => $e->getMessage()], 500);
        }
    }
     ...
     * e.g., /?controller=Stock&action=update&id=...
     */
    public function update(Request $request): Response
    {
        ...
        try {
            $item = $this->service->PutStock($id, $data);
            if (!$item) {
                return $this->json(['error' => 'Stock item not found.'], 404);
            }
            return $this->json($item);
        } catch (Exception $e) {
            return $this->json(['error' => 'Failed to update stock item.', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function updateQuantity(Request $request): Response
    {
        ...
        try {
            $item = $this->service->PutQuantity($id, (float)$quantity);
            if (!$item) {
                return $this->json(['error' => 'Stock item not found.'], 404);
            }
            return $this->json($item);
    ...
    /**
     * Deletes (deactivates) a stock item by its ID.
     * e.g., /?controller=Stock&action=destroy&id=...
     */
    public function destroy(Request $request): Response
    {
        ...
        try {
            $success = $this->service->DeleteStock($id);
            if ($success) {
                return new Response('', 204); // 204 No Content
            }
            return $this->json(['error' => 'Failed to delete stock item.'], 500);
        } catch (Exception $e) {
